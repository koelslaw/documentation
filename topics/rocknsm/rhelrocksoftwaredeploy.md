...
A thing to not there are 3 elastic virtual machines in ESXi. 1 of the 3 will have kibana while the others will just have logstash and elasticsearch. We only need 1 kibana for all the analyst to reach.  

The elesticsearch nodes will coordinate with each other to share the data. We can keep adding elastic resources if hardware will allow it.
...
# FIRE ZE LAZORZZZZ!!!!
The ansible script doesnt play nicely with the with the shell script wrapper. So we will deploy the script directly.
